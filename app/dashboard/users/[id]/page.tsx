...
export default function UserDetails() {
  ...
  const { data: activityResp, isLoading: activityLoading } =
    useGetUserActivityQuery({ userId });
  const { credit, isLoading: creditLoading } = useCreditUserWallet();
  const { debit, isLoading: debitLoading } = useDebitUserWallet();
  ...
  const totalSpent = shipments.reduce(
    (sum: number, s: any) => sum + (s?.ordervalue ?? s?.totalprice ?? 0),
    0
  );
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            >
                ...
                >
                  <div className="flex flex-col gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-4xl font-bold text-gray-900 tabular-nums break-words max-w-full">
                    ...
                    <div className="mt-3 flex gap-2">
                      <button
                        ...
                        onClick={async () => {
                          const amountStr =
                            window.prompt(
                              "أدخل المبلغ المطلوب إضافته (ريال):"
                            ) || "";
                          ...
                          const note =
                            window.prompt("ملاحظة (اختياري):") || undefined;
                          try {
                      ...
                      <button
                        ...
                        onClick={async () => {
                          const amountStr =
                            window.prompt("أدخل المبلغ المطلوب خصمه (ريال):") ||
                            "";
                          const amount = parseFloat(amountStr);
                          if (!amount || amount <= 0 || !isFinite(amount)) {
                            alert("الرجاء إدخال مبلغ صحيح");
                          ...
                          const note =
                            window.prompt("ملاحظة (اختياري):") || undefined;
                          try {
                            await debit({ userId, amount, description: note });
                            await refetchWallet();
                            alert("تم خصم الرصيد بنجاح");
                          } catch (e: any) {
                            alert(e?.message || "فشل خصم الرصيد");
                          }
                        }}
              ...
              <motion.div
                ...
                style={{
                  boxShadow:
                    "0 25px 50px -12px rgba(0, 0, 0, 0.1), 0 8px 16px -8px rgba(0, 0, 0, 0.05)",
                }}
              >
                  ...
                  {shipments.length ? (
                    ...
                    <div className="text-center py-12">
                      ...
                      <p className="text-gray-500">
                        لم يقم المستخدم بإنشاء أي شحنات بعد
                      </p>
                    </div>
                  )}
                </div>
